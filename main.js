...
UI.prototype.draw = function (ctx) { //draw ui
    ...
    ctx.fillText("Homecoming  " + Math.floor(this.game.player1.currentHealth) + " / " + this.game.player1.maxHealth,this.portraitX + 90 + this.game.liveCamera.x,this.portraitY + 30 + this.game.liveCamera.y);
    ctx.fillText("Score: " + this.game.score, this.scoreX + this.game.liveCamera.x, this.scoreY - 15 + this.game.liveCamera.y);
	...
	if (this.life > 0) {
		...
		if (this.life <= 0) {
			...
			if (this.game.currentPhase === 3) { //i sense something...
				...
				var chat = new TextBox(this.game, "./img/Chat/HocoSquare.png", "Who is the one griefing again?");
		...
		if (this.type === 7) { //tree of life
	        ...
	        if (checkCollision(this, this.game.player1)) {
				...
				if (this.gatherTime >= 180) {
					...
					var chat = new TextBox(this.game, "./img/Chat/HocoSquare.png", "There's a suspicious smell nearby.");
					this.game.addEntity(chat);
...
function Character(game) {
	...
	//function Animation(spriteSheet, startX, startY, frameWidth, frameHeight, frameDuration, frames, loop, reverse, offsetX, offsetY)
    this.idleAnimationRight = new Animation(ASSET_MANAGER.getAsset("./img/Hoco/hoco.png"), 0, 0, 64, 64, 1, 1, true, false, 0, 0);
    this.idleAnimationLeft = new Animation(ASSET_MANAGER.getAsset("./img/Hoco/hocoleft.png"), 0, 0, 64, 64, 1, 1, true, false, 0, 0);
	this.idleAnimation = this.idleAnimationRight;
    
    // Hurt
    this.hurtAnimationRight = new Animation(ASSET_MANAGER.getAsset("./img/Hoco/hoco.png"), 0, 0, 64, 64, 1, 1, false, false, 0, 0);
    this.hurtAnimationLeft = new Animation(ASSET_MANAGER.getAsset("./img/Hoco/hocoleft.png"), 0, 0, 64, 64, 1, 1, false, false, 0, 0);
    ...
    this.hitBoxDef = {
    	width: 32, height: 32, offsetX: 16, offsetY: 16, growthX: 0, growthY: 0, originalOffsetX: 0
    };
    drawHitBox(this);
}
...
Character.prototype.update = function () {
    ...
    if (gameStarted) {
		...
		if (this.game.currentPhase < 1) {
			startMusic.play();
			var chat = new TextBox(this.game, "./img/Chat/HocoSquare.png", "bork bork bork bork bork");
			this.game.step = 0;
			this.game.addEntity(chat);
            this.game.currentPhase = 1;
		}
			...
			if (this.phaseTimer === 0) {
				switch(this.game.currentPhase) {
					...
					case 6: //brandong tp
						...
						bossMusic.play();
						var chat = new TextBox(this.game, "./img/Chat/HocoSquare.png", "Are you serious?");
						this.game.addEntity(chat);
		...
		if (this.x >= 1100) {
			if (this.game.currentPhase === 1) {
				...
				var chat = new TextBox(this.game, "./img/Chat/HocoSquare.png", "What's this? Free tree?");
				this.game.addEntity(chat);
				this.game.player1.canControl = false;
			}
		}
        ...
        if (!this.dead) {
            if (!this.vulnerable) {
				if (this.xVelocity !== 0 || this.yVelocity !== 0) { //knockback equates to stun
			...
			if (this.zoomTimer > 0) { //speed gate
				...
				if (this.zoomTimer === 0) {
					this.xVelocity = 0;
					var chat = new TextBox(this.game, "./img/Chat/HocoSquare.png", "I think I lost him for now...");
			...
			if (this.speedTimer > 0) {
				...
				if (this.speedTimer % 8 == 0)
					this.game.addEntity(new Particle(IMG_PART, this.x, this.y, 0, 0, 0, 0, 0, 0, 0, 10, 0, 10, 0.5, 0, false, this.game,
							this.lastDirection === "Right" ? new Animation(ASSET_MANAGER.getAsset("./img/Hoco/hoco.png"), 0, 0, 64, 64, 1, 10, true, false, 0, 0) : 
							new Animation(ASSET_MANAGER.getAsset("./img/Hoco/hocoleft.png"), 0, 0, 64, 64, 1, 10, true, false, 0, 0)));
			}
            ...
            if ((this.rightDown || this.leftDown || this.jumpDown || this.downDown) && this.canControl) {
                if (this.rightDown) {
					this.running = true;
                    this.lastDirection = "Right";
				...
				if (this.jumpDown) {
					this.runningVertical = true;
                    this.lastDirectionVertical = "Up";
                } else if (this.downDown) {
					this.runningVertical = true;
                    this.lastDirectionVertical = "Down";
                } else {
					this.runningVertical = false;
				}
            } else {
...
function spawnWave(game, number) {
	switch(number) {
		case 1:
			...
			var enemies = [new Chicken(game, -1512, 208, 0, 0, 0, 0, 0),

new Chicken(game, -1448, 144, 0, 0, 0, 0, 0),

new Chicken(game, -1384, 96, 0, 0, 0, 0, 0),

new Chicken(game, -1448, 272, 0, 0, 0, 0, 0),

new Chicken(game, -1384, 320, 0, 0, 0, 0, 0),

new Chicken(game, -1320, 368, 0, 0, 0, 0, 0),

new Chicken(game, -1320, 48, 0, 0, 0, 0, 0),

new Chicken(game, -728, 224, 0, 0, 0, 0, 0),

new Chicken(game, -856, 256, 0, 0, 0, 0, 0),

new Chicken(game, -904, 352, 0, 0, 0, 0, 0),

new Chicken(game, -808, 400, 0, 0, 0, 0, 0),

new Chicken(game, -696, 400, 0, 0, 0, 0, 0),

new Chicken(game, -776, 320, 0, 0, 0, 0, 0),

new Chicken(game, -680, 320, 0, 0, 0, 0, 0),

new Chicken(game, -744, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -712, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -680, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -200, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -152, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -104, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -72, 144, 0, 0, 0, 0, 0),

new Chicken(game, -136, 240, 0, 0, 0, 0, 0),

new Chicken(game, 232, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 280, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 328, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 376, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 424, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -600, 368, 0, 0, 0, 0, 0),

new Chicken(game, -200, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -152, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -104, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -56, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -8, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -56, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -8, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 360, 272, 0, 0, 0, 0, 1),

new Chicken(game, 408, 272, 0, 0, 0, 0, 1),

new Chicken(game, 456, 272, 0, 0, 0, 0, 1),

new Chicken(game, 504, 272, 0, 0, 0, 0, 1),

new Chicken(game, 552, 272, 0, 0, 0, 0, 1),

new Chicken(game, 600, 272, 0, 0, 0, 0, 1),

new Chicken(game, 216, 160, 0, 0, 0, 0, 0),

new Chicken(game, 120, 128, 0, 0, 0, 0, 0),

new Chicken(game, 88, 352, 0, 0, 0, 0, 0),

new Chicken(game, -1256, 0, 0, 0, 0, 0, 0),

new Chicken(game, -1256, 400, 0, 0, 0, 0, 0),

new Chicken(game, -1144, 64, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -1144, 384, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -1112, 128, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -1080, 192, 0, 0, -0.10, -0.10, 0),

new Chicken(game, -1080, 256, 0, 0, -0.10, 0.10, 0),

new Chicken(game, -1112, 320, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 776, 208, 0, 0, 0, 0, 1),

new Chicken(game, 824, 208, 0, 0, 0, 0, 1),

new Chicken(game, 872, 208, 0, 0, 0, 0, 1),

new Chicken(game, 920, 208, 0, 0, 0, 0, 1),

new Chicken(game, 968, 208, 0, 0, 0, 0, 1),

new Chicken(game, 2504, 0, 0, 0, 0, 0, 0),

new Chicken(game, 2488, 64, 0, 0, 0, 0, 0),

new Chicken(game, 2472, 128, 0, 0, 0, 0, 0),

new Chicken(game, 2456, 192, 0, 0, 0, 0, 0),

new Chicken(game, 2456, 256, 0, 0, 0, 0, 0),

new Chicken(game, 2472, 320, 0, 0, 0, 0, 0),

new Chicken(game, 2488, 384, 0, 0, 0, 0, 0),

new Chicken(game, 2504, 448, 0, 0, 0, 0, 0),

new Chicken(game, 2600, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 2648, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 2696, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 2744, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 2792, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 3368, 416, 0, 0, 0, 0, 0),

new Chicken(game, 3608, 320, 0, 0, 0, 0, 1),

new Chicken(game, 3672, 304, 0, 0, 0, 0, 1),

new Chicken(game, 3736, 288, 0, 0, 0, 0, 1),

new Chicken(game, 3800, 272, 0, 0, 0, 0, 1),

new Chicken(game, 3864, 256, 0, 0, 0, 0, 1),

new Chicken(game, 3928, 240, 0, 0, 0, 0, 1),

new Chicken(game, 3992, 224, 0, 0, 0, 0, 1),

new Chicken(game, 5848, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 5896, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 5944, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 5992, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 6040, 0, 0, 0, -0.10, 0.10, 0),

new Chicken(game, 5848, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 5896, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 5944, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 5992, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 6040, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 7336, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 7384, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 7432, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 7480, 432, 0, 0, -0.10, -0.10, 0),

new Chicken(game, 7528, 432, 0, 0, -0.10, -0.10, 0),

new Boar(game, -520, 192),

new Boar(game, -248, 64),

new Boar(game, -360, 336),

new Boar(game, 8, 208),

new Boar(game, 360, 16),

new Boar(game, 264, 48),

new Boar(game, 344, 80),

new Boar(game, -520, 128),

new Boar(game, 8, 256),

new Boar(game, 440, 96),

new Boar(game, 488, 32),

new Boar(game, 584, 112),

new Boar(game, 616, 0),

new Boar(game, 664, 64),

new Boar(game, 3096, 128),

new Boar(game, 3096, 192),

new Boar(game, 3096, 256),

new Boar(game, 3096, 320),

new Boar(game, 3096, 384),

new Boar(game, 3096, 448),

new Boar(game, 3352, 0),

new Boar(game, 3352, 64),

new Boar(game, 3352, 128),

new Boar(game, 3352, 192),

new Boar(game, 3352, 256),

new Boar(game, 3352, 320),

new Boar(game, 5144, 112),

new Boar(game, 4872, 272),

new Boar(game, 4872, 208),

new Boar(game, 5144, 48),

new Boar(game, 5544, 144),

new Boar(game, 5544, 208),

new Boar(game, 6200, 160),

new Boar(game, 6200, 224),

new Boar(game, 6296, 160),

new Boar(game, 6296, 224),

new Boar(game, 6392, 160),

new Boar(game, 6392, 224),

new Boar(game, 6296, 96),

new Boar(game, 6392, 96),

new Boar(game, 6296, 288),

new Boar(game, 6392, 288),

new Boar(game, 6392, 32),

new Boar(game, 6392, 352),

new Boar(game, 6488, 416),

new Boar(game, 6488, 352),

new Boar(game, 6488, 288),

new Boar(game, 6488, 224),

new Boar(game, 6488, 160),

new Boar(game, 6488, 96),

new Boar(game, 6488, 32),

new Boar(game, 6488, -32),

new Boar(game, 6120, 192),

new Boar(game, 7464, 0),

new Boar(game, 7576, 0),

new Powerup(game, -200, 240, 6),

new Powerup(game, 4488, 64, 6),

new Powerup(game, 5736, 208, 6),

new Powerup(game, 1048, 224, 0),

new Powerup(game, 2920, 400, 0),

new Powerup(game, 5256, 64, 0),

new Powerup(game, 2264, 240, 5),

new Powerup(game, 3128, 48, 5),

new Powerup(game, 6632, 432, 5),

new Powerup(game, 16328, 224, 5),

new Powerup(game, 4248, 96, 8),

new Powerup(game, 4312, 224, 8),

new Powerup(game, 4424, 144, 8),

new Powerup(game, 4424, 320, 8),

new Powerup(game, 4216, 320, 8),

new Powerup(game, 4232, 464, 8),

new Powerup(game, 4328, 400, 8),

new Powerup(game, 4520, 400, 8),

new Powerup(game, 4632, 16, 8),

new Powerup(game, 4696, 96, 8),

new Powerup(game, 4824, 32, 8),

new Powerup(game, 4648, 416, 8),

new Powerup(game, 4808, 144, 8),

new Powerup(game, 4552, 256, 8),

new Powerup(game, 4744, 336, 8),

new Powerup(game, 4856, 416, 8),

new Powerup(game, 4968, 16, 8),

new Powerup(game, 5000, 368, 8),

new Powerup(game, 5080, 240, 8),

new Powerup(game, 5208, 400, 8),

new Powerup(game, 5224, 288, 8),

new Powerup(game, 5256, 160, 8),

new Powerup(game, 5368, 352, 8),

new Powerup(game, 5400, 0, 8),

new Powerup(game, 5528, 48, 8),

new Powerup(game, 5384, 224, 8),

new Powerup(game, 5512, 320, 8),

new Powerup(game, 5672, 400, 8),

new Powerup(game, 5688, 0, 8),

new Powerup(game, 6760, 432, 8),

new Powerup(game, 6760, 368, 8),

new Powerup(game, 6760, 304, 8),

new Powerup(game, 6760, 240, 8),

new Powerup(game, 6760, 176, 8),

new Powerup(game, 6760, 112, 8),

new Powerup(game, 6904, 0, 8),

new Powerup(game, 6904, 64, 8),

new Powerup(game, 6904, 128, 8),

new Powerup(game, 6904, 192, 8),

new Powerup(game, 6904, 256, 8),

new Powerup(game, 6904, 320, 8),

new Powerup(game, 6968, 320, 8),

new Powerup(game, 6968, 256, 8),

new Powerup(game, 6968, 192, 8),

new Powerup(game, 6968, 128, 8),

new Powerup(game, 6968, 64, 8),

new Powerup(game, 6968, 0, 8),

new Powerup(game, 6696, 112, 8),

new Powerup(game, 6696, 176, 8),

new Powerup(game, 6696, 240, 8),

new Powerup(game, 6696, 304, 8),

new Powerup(game, 6696, 368, 8),

new Powerup(game, 6696, 432, 8),

new Powerup(game, 7128, 432, 8),

new Powerup(game, 7192, 432, 8),

new Powerup(game, 7128, 368, 8),

new Powerup(game, 7192, 368, 8),

new Powerup(game, 7128, 304, 8),

new Powerup(game, 7192, 304, 8),

new Powerup(game, 7128, 240, 8),

new Powerup(game, 7192, 240, 8),

new Powerup(game, 7128, 176, 8),

new Powerup(game, 7192, 176, 8),

new Powerup(game, 7128, 112, 8),

new Powerup(game, 7192, 112, 8),

new Powerup(game, 4120, 160, 9),

new Powerup(game, 4120, 240, 9),

new Powerup(game, 4120, 320, 9),

new Powerup(game, 4120, 80, 9),

new Powerup(game, 6632, 32, 9),

new Powerup(game, 7736, 0, 10),
...
ASSET_MANAGER.queueDownload("./img/Chat/HocoSquare.png");
ASSET_MANAGER.queueDownload("./img/Chat/BrandongSquare.png");

ASSET_MANAGER.queueDownload("./img/Hoco/hoco.png");
ASSET_MANAGER.queueDownload("./img/Hoco/hocoleft.png");
